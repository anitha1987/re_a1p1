...
this.uiDialog.position(position);if(!isVisible){this.uiDialog.hide();}}});}}(jQuery));(function(jQuery){jQuery.hotkeys={version:"0.8",specialKeys:{8:"backspace",9:"tab",13:"return",16:"shift",17:"ctrl",18:"alt",19:"pause",20:"capslock",27:"esc",32:"space",33:"pageup",34:"pagedown",35:"end",36:"home",37:"left",38:"up",39:"right",40:"down",45:"insert",46:"del",96:"0",97:"1",98:"2",99:"3",100:"4",101:"5",102:"6",103:"7",104:"8",105:"9",106:"*",107:"+",109:"-",110:".",111:"/",112:"f1",113:"f2",114:"f3",115:"f4",116:"f5",117:"f6",118:"f7",119:"f8",120:"f9",121:"f10",122:"f11",123:"f12",144:"numlock",145:"scroll",186:";",191:"/",220:"\\",222:"'",224:"meta"},shiftNums:{"`":"~","1":"!","2":"@","3":"#","4":"$","5":"%","6":"^","7":"&","8":"*","9":"(","0":")","-":"_","=":"+",";":": ","'":"\"",",":"<",".":">","/":"?","\\":"|"}};function keyHandler(handleObj){if(typeof handleObj.data==="string"){handleObj.data={keys:handleObj.data};}
...
jQuery.each(["keydown","keyup","keypress"],function(){jQuery.event.special[this]={add:keyHandler};});})(jQuery);this.Handlebars={};(function(Handlebars){Handlebars.VERSION="1.0.0-rc.3";Handlebars.COMPILER_REVISION=2;Handlebars.REVISION_CHANGES={1:'<= 1.0.rc.2',2:'>= 1.0.0-rc.3'};Handlebars.helpers={};Handlebars.partials={};Handlebars.registerHelper=function(name,fn,inverse){if(inverse){fn.not=inverse;}
...
util.on(global,'load',fn,false);};util.on=function(element,event,fn,capture){if(element.attachEvent){element.attachEvent('on'+event,fn);}else if(element.addEventListener){element.addEventListener(event,fn,capture);}};util.request=function(xdomain){if(xdomain&&'undefined'!=typeof XDomainRequest&&!util.ua.hasCORS){return new XDomainRequest();}
...
ret.push(shortest[i]);}
return ret;};util.indexOf=function(arr,o,i){for(var j=arr.length,i=i<0?i+j<0?0:i+j:i||0;i<j&&arr[i]!==o;i++){}
...
return this;};EventEmitter.prototype.removeAllListeners=function(name){if(name===undefined){this.$events={};return this;}
...
var self=this;self.connecting=true;this.handshake(function(sid,heartbeat,close,transports){self.sessionid=sid;self.closeTimeout=close*1000;self.heartbeatTimeout=heartbeat*1000;if(!self.transports)
self.transports=self.origTransports=(transports?io.util.intersect(transports.split(','),self.options.transports):self.options.transports);self.setHeartbeatTimeout();function connect(transports){if(self.transport)self.transport.clearTimeouts();self.transport=self.getTransport(transports);if(!self.transport)return self.publish('connect_failed');self.transport.ready(self,function(){self.connecting=true;self.publish('connecting',self.transport.name);self.transport.open();if(self.options['connect timeout']){self.connectTimeoutTimer=setTimeout(function(){if(!self.connected){self.connecting=false;if(self.options['try multiple transports']){var remaining=self.transports;while(remaining.length>0&&remaining.splice(0,1)[0]!=self.transport.name){}
...
if(self.reconnectionAttempts++>=maxAttempts){if(!self.redoTransports){self.on('connect_failed',maybeReconnect);self.options['try multiple transports']=true;self.transports=self.origTransports;self.transport=self.getTransport();self.redoTransports=true;self.connect();}else{self.publish('reconnect_failed');reset();}}else{if(self.reconnectionDelay<limit){self.reconnectionDelay*=2;}
...
script.async=true;script.src=this.prepareUrl()+query;script.onerror=function(){self.onClose();};var insertAt=document.getElementsByTagName('script')[0];insertAt.parentNode.insertBefore(script,insertAt);this.script=script;if(indicator){setTimeout(function(){var iframe=document.createElement('iframe');document.body.appendChild(iframe);document.body.removeChild(iframe);},100);}};JSONPPolling.prototype._=function(msg){this.onData(msg);if(this.isOpen){this.get();}
return this;};JSONPPolling.prototype.ready=function(socket,fn){var self=this;if(!indicator)return fn.call(this);io.util.load(function(){fn.call(self);});};JSONPPolling.check=function(){return'document'in global;};JSONPPolling.xdomainCheck=function(){return true;};io.transports.push('jsonp-polling');})('undefined'!=typeof io?io.Transport:module.exports,'undefined'!=typeof io?io:module.parent.exports,this);if(typeof define==="function"&&define.amd){define([],function(){return io;});}})();(function(){var
...
_publish(channel,payload);},reInit:function(){_reloadData();}};_init();})();(function($,global){'use strict';var templates=null;Handlebars.registerHelper('join',function(context,opts){return context.join(', ');});Handlebars.registerHelper('date',function(context,opts){var ts=moment(context);return ts.format(opts.hash.format||'YYYY-MM-DD HH:mm:ss');});function loadTemplates(){if(!$.isReady){console.error('Tried loading templates before DOMReady');console.trace();return false;}
templates={};$('script.template').each(function(){var $this=$(this);templates[$this.data('id')]=Handlebars.compile($this.html().trim());}).remove();return true;}
global.renderTemplate=function renderTemplate(name,context){if(templates===null&&!loadTemplates()){return'Cannot use templates before DOMReady. ';}
var tplFunc=templates[name]||function(){console.error('Template not found: '+name);console.trace();return'Template "'+name+'" not found. ';};return $($.parseHTML(tplFunc(context||{})));};})(jQuery,window);(function($,global){'use strict';global.restCall=function restCall(method,path){return $.ajax({url:path,type:method}).fail(function(xhr,status,error){alert('REST call failed: '+method+' '+path+'\nStatus: '+status+'\nError: '+error);});};})(jQuery,window);(function($,global){'use strict';var Aborter=global.Aborter=function Aborter(){this.aborted=$.Deferred();};Aborter.prototype={abort:function abort(){this.aborted.reject();this.aborted=$.Deferred();},watch:function watch(deferred){var newDeferred=$.Deferred();deferred.then(function(){newDeferred.resolveWith(this,Array.prototype.slice.call(arguments));},function(){newDeferred.rejectWith(this,Array.prototype.slice.call(arguments));});this.aborted.fail(function(){newDeferred.rejectWith(this,Array.prototype.slice.call(arguments));});return newDeferred;}};})(jQuery,window);(function($,global){'use strict';global.registerHotkeys=function registerHotkeys(map){$.each(map,function(key,fn){$(document).on('keydown',null,key,fn);});};})(jQuery,window);(function($,global){'use strict';var permissionMap={'granted':true,'default':undefined,'denied':false,0:true,1:undefined,2:false};function show(title,text,opts,timeout,onclick){opts=$.extend({},opts||{});opts.body=text;var notification=new Notification(title,opts);if(onclick){notification.onclick=onclick;}
if(timeout){window.setTimeout(function(){notification.close();},timeout);}
return function(){notification.close();};}
function checkPermission(){var perm=Notification.permission;if(perm===undefined&&window.webkitNotifications){perm=window.webkitNotifications.checkPermission();}
return permissionMap[perm];}
function requestPermission(callback){Notification.requestPermission(function(response){callback(permissionMap[response]);});}
if(window.Notification){global.NotificationUtil={available:true,show:show,checkPermission:checkPermission,requestPermission:requestPermission};}
else{global.NotificationUtil={available:false};}})(jQuery,window);(function($,global){'use strict';var messages={};var cleared=new Aborter();var filterTerm='';function addCacheBuster(formats){$.each(formats,function(format,url){formats[format]=url+'?_='+(new Date().getTime());});return formats;}
var Message=global.Message=function Message(msg,loadedEverything){this._loaded=loadedEverything||false;this._deleted=false;this.id=msg.id;this.sender=msg.sender;this.recipients=msg.recipients;this.created_at=new Date(msg.created_at);this.subject=msg.subject;this.size=msg.size;if(this._loaded){this.href=msg.href;this.formats=addCacheBuster(msg.formats);this.attachments=msg.attachments;}
else{this.href='#';this.attachments=[];this.formats={};}};Message.prototype={dom:function(){return this._dom||(this._dom=renderTemplate('message',this));},display:function(){var self=this;$('#message-metadata').html(renderTemplate('message-metadata',this));$('.action.download a').attr('href',this.href);$('.views .format').each(function(){var $this=$(this);var format=$this.data('messageFormat');$this.toggle(format in self.formats);$('a',this).attr('href',self.formats[format]||'#');}).removeClass('selected').filter(':visible:first').addClass('selected');this.updateFormat();},updateFormat:function(){var format=$('.views .format.selected').data('messageFormat');if($('#message-body').attr('src')!=this.formats[format]){$('#message-body').attr('src',this.formats[format]);}},del:function(){delete messages[this.id];this.selectSibling();this.closeNotification();if(this._dom){this._dom.remove();delete this._dom;}},delRemote:function(){var self=this;if(this._deleted){return;}
this._deleted=true;this.dom().addClass('deleted');this.selectSibling();this.closeNotification();restCall('DELETE','/messages/'+this.id).fail(function(){self._deleted=false;self.dom().removeClass('deleted');});},selected:function(){return $('#messages > .selected').data('messageId')==this.id;},selectSibling:function(){if(!this._dom||!this.selected()){return;}
this.deselect();var sibling=this._dom.nextAll(':visible:not(.deleted)').first();if(!sibling.length){sibling=this._dom.prevAll(':visible:not(.deleted)').first();}
sibling.trigger('click');},select:function(){if(!this._dom){console.error('Cannot select message that has not been rendered.');}
var row=this.dom();this.closeNotification();$('#message').removeClass('no-message').addClass('loading-message');$('#messages > tr.selected').removeClass('selected');row.addClass('selected');if(row.position().top<=0||row.position().top+row.height()>row.offsetParent().height()){if(row.index()==0){row.closest('table').find('thead')[0].scrollIntoView();}
else{row[0].scrollIntoView();}}
$('#message-body').attr('src','about:blank');this.load().done(function(){$('#message').removeClass('loading-message');if(this.selected()){this.display();}});},deselect:function(){if(!this._dom){return;}
this._dom.removeClass('selected');$('#message').addClass('no-message');$('#message-body').attr('src','about:blank');},load:function(){var self=this;var deferred=$.Deferred();if(this._loaded){deferred.resolveWith(this);}
else{cleared.watch(restCall('GET','/messages/'+this.id+'.json')).done(function(data){self._loaded=true;self.href=data.href;self.attachments=data.attachments;self.formats=addCacheBuster(data.formats);deferred.resolveWith(self);});}
return deferred.promise();},showNotification:function(){var self=this;var msg='From '+this.sender+'\xa0 to \xa0'+this.recipients.to.join(', ');this._closeNotification=NotificationUtil.show(this.subject,msg,{icon:'/static/images/icon_80x80.png'},10000,function(){self.select();});},closeNotification:function(){if(this._closeNotification){this._closeNotification();delete this._closeNotification;}}};Message.get=function(id){return messages[id];};Message.getSelected=function(){return Message.get($('#messages > .selected').data('messageId'));};Message.deleteAll=function(){Message.closeNotifications();$('#messages > tr').remove();$('#message').addClass('no-message');messages={};cleared.abort();};Message.closeNotifications=function(){$.each(messages,function(id,message){message.closeNotification();});};Message.load=function(id,notify){cleared.watch(restCall('GET','/messages/'+id+'.json')).done(function(msg){var message=Message.add(msg,true);Message.applyFilter();if(notify){message.showNotification();}});};Message.add=function(msg,loadedEverything){if(msg.id in messages){console.warn('Message '+msg.id+' already exists.');return;}
messages[msg.id]=new Message(msg,loadedEverything);$('#messages').prepend(messages[msg.id].dom());return messages[msg.id];};Message.loadAll=function(){Message.deleteAll();$('#loading-dialog').dialog('open');restCall('GET','/messages/').done(function(data){$.each(data.messages,function(i,msg){Message.add(new Message(msg));});Message.applyFilter();}).always(function(){$('#loading-dialog').dialog('close');});};Message.applyFilter=function(term){if(term!==undefined){filterTerm=term;}
var all=$('#messages > tr').show();if(filterTerm){all.filter(function(){return!~$(this).text().toLowerCase().indexOf(filterTerm);}).hide();var selected=Message.getSelected();if(selected&&!selected.dom().is(':visible')){selected.deselect();}}}})(jQuery,window);(function($){'use strict';$(document).ready(function(){$('.resizer').on('mousedown',function(e){if(e.button!=0){return;}
var $this=$(this);var target=$this.data('sibling')=='prev'?$this.prev():$this.next();e.preventDefault();$(document).on('mousemove.resizer',function(e){e.preventDefault();target.css('height',e.clientY-target.offset().top);}).on('mouseup.resizer',function(e){e.preventDefault();$(document).off('.resizer');});});$('#disconnected-dialog, #loading-dialog').dialog({autoOpen:false,closeOnEscape:false,dialogClass:'no-close',draggable:false,modal:true,resizable:false,title:''});$('nav.app .quit a').on('click',function(e){e.preventDefault();if(!confirm('Do you really want to terminate the MailTrap application?')){return;}
restCall('DELETE','/');});$('nav.app .clear a').on('click',function(e){e.preventDefault();if(!confirm('Do you really want to delete all messages?')){return;}
restCall('DELETE','/messages/');});if(NotificationUtil.available){var notificationButton=$('nav.app .notifications a');var updateNotificationButton=function updateNotificationButton(){var enabled=$.jStorage.get('notifications')&&NotificationUtil.checkPermission();notificationButton.text(enabled?'Disable notifications':'Enable notifications');};notificationButton.parent().show();notificationButton.on('click',function(e){e.preventDefault();switch(NotificationUtil.checkPermission()){case false:alert('You need to allow notifications via site permissions.');return;case true:$.jStorage.set('notifications',!$.jStorage.get('notifications'));updateNotificationButton();break;default:NotificationUtil.requestPermission(function(perm){$.jStorage.set('notifications',!!perm);updateNotificationButton();});break;}});updateNotificationButton();}
$('#search').on('keyup',function(){var term=$(this).val().trim().toLowerCase();Message.applyFilter(term);});$('#messages').on('click','> tr:not(.deleted)',function(e){var msg;if(e.ctrlKey){msg=Message.getSelected();if(msg){msg.deselect();}
if(window.getSelection){window.getSelection().removeAllRanges();}}
else{msg=Message.get($(this).data('messageId'));if(msg&&msg!=Message.getSelected()){msg.select();$('#message').show();}}});$('.tab.format').on('click',function(e){e.preventDefault();var msg=Message.getSelected();if(msg){$('.tab.format.selected').removeClass('selected');$(this).addClass('selected');msg.updateFormat();}});$('.action.delete').on('click',function(e){e.preventDefault();var msg=Message.getSelected();if(msg){msg.delRemote();}});Message.loadAll();var socket=io.connect(undefined,{'reconnection limit':10000,'max reconnection attempts':Infinity});var terminating=false;window.onbeforeunload=function(){terminating=true;socket.disconnect();Message.closeNotifications()};socket.on('connect',function(){$('#disconnected-dialog').dialog('close');}).on('reconnect',function(){Message.loadAll();}).on('disconnect',function(){if(terminating){return;}
$('#loading-dialog').dialog('close');$('#disconnected-dialog').dialog('open');}).on('add_message',function(id){Message.load(id,$.jStorage.get('notifications'));}).on('delete_message',function(id){var msg=Message.get(id);if(msg){msg.del();}}).on('delete_messages',function(){Message.deleteAll();});registerHotkeys({'del':function(){var msg=Message.getSelected();if(msg){msg.delRemote();}},'backspace':function(e){e.preventDefault();},'f5':function(){Message.closeNotifications();},'ctrl+f5':function(){Message.closeNotifications();},'ctrl+r':function(){Message.closeNotifications();},'up':function(e){e.preventDefault();var msg=Message.getSelected();if(!msg){$('#messages > tr:last').trigger('click');return;}
msg.dom().prevAll(':visible').first().trigger('click');},'down':function(e){e.preventDefault();var msg=Message.getSelected();if(!msg){$('#messages > tr:first').trigger('click');return;}
msg.dom().nextAll(':visible').first().trigger('click');},'ctrl+up':function(e){e.preventDefault();$('#messages > tr:first').trigger('click');},'ctrl+down':function(e){e.preventDefault();$('#messages > tr:last').trigger('click');},'/':function(e){e.preventDefault();$('#search').focus();}});});})(jQuery);